...
public class PuzzleBackSpecial : MonoBehaviour
{
    // 动画控制器组件
    ...
    void Start()
    {
        // 初始化禁用所有Canvas（硬编码前5个元素）
        for (int i = 0; i < 5; i++)
            targetCanvas[i].enabled = false;
    ...
    private void Update()
    {
        ...
        if (stateInfo.IsName("Scene") && stateInfo.normalizedTime >= 0.99f)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                {
                    targetCanvas[i].enabled = true; // 启用Canvas组件
                }
            }
        }
    }
